...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('products-add')) {
            ...
            return view('backend.product.create', compact('lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_warehouse_list', 'numberOfProduct', 'custom_fields', 'extraCategories'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        $data['second_name'] = preg_replace('/[\n\r]/', "<br>", htmlspecialchars(trim($data['sec-name']), ENT_QUOTES));
        ...
        Log::debug($request->extra_categories);
        if ($request->extra_categories) {
            foreach ($request->extra_categories as $category) {
                ProductExtraCategory::create([
                    'product_id' => $lims_product_data->id,
                    'extra_category_id' => $category,
                ]);
            }
        }
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('products-edit')) {
            ...
            if ($lims_product_data->variant_option != null) {
            ...
            if (in_array('ecommerce', explode(',', config('addons')))) {
                ...
                return view('backend.product.edit', compact('related_products', 'lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'lims_warehouse_list', 'noOfVariantValue', 'custom_fields', 'extraCategories','selectedCategories'));
            }

            //dd($selectedCategories);
            return view('backend.product.edit', compact('lims_product_list_without_variant', 'lims_product_list_with_variant', 'lims_brand_list', 'lims_category_list', 'lims_unit_list', 'lims_tax_list', 'lims_product_data', 'lims_product_variant_data', 'lims_warehouse_list', 'noOfVariantValue', 'custom_fields', 'extraCategories','selectedCategories'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function updateProduct(Request $request)
    {
        ...
        $data['second_name'] = htmlspecialchars(trim($data['sec-name']), ENT_QUOTES);
        ...
        //Log::debug($request->extra_categories);
        if ($request->extra_categories) {
            ProductExtraCategory::where('product_id',$lims_product_data->id)->delete();
            foreach ($request->extra_categories as $category) {
                Log::debug($category);
                ProductExtraCategory::create([
                    'product_id' => $lims_product_data->id,
    ...
    public function extra()
    {
        ...
        if ($role->hasPermissionTo('products-index')) {
            ...
            if (empty($all_permission))
                $all_permission[] = 'dummy text';
            $role_id = $role->id;
            $numberOfProduct = DB::table('extra_categories')->count();
            ...
            return view('backend.product.extras', compact('all_permission', 'role_id', 'numberOfProduct', 'custom_fields', 'field_name'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    //     $totalData = DB::table('extra_categories')->count();
    //     $totalFiltered = $totalData;
    ...
    //         $limit = $totalData;
    //     $start = $request->input('start');
    //     $order = 'extra.' . $columns[$request->input('order.0.column')];
    //     $dir = $request->input('order.0.dir');
    //     //fetching custom fields data
    //     $custom_fields = CustomField::where([
    //         ['belongs_to', 'product'],
    ...
    //     }
    //     if (empty($request->input('search.value'))) {
    //         $products = ExtraCategory::offset($start)
    ...
    //     } else {
    //         $search = $request->input('search.value');
    //         $q = ExtraCategory::select('extra_categories.*')

    //             ->where([
    //                 ['extra_categories.category_name', 'LIKE', "%{$search}%"]
    ...
    //             $nestedData['name'] = $product->category_name;
    ...
    //     $role = Role::find(Auth::user()->role_id);
    //     if ($role->hasPermissionTo('products-index')) {
    //         return view('backend.product.extras');
    //     } else
    //         return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    // }
    ...
    public function extraData(Request $request)
    {
        ...
        $dir = $request->input('order.0.dir');
        if (empty($request->input('search.value')))
            $categories = ExtraCategory::offset($start)
            ...
            $search = $request->input('search.value');
            $categories =  ExtraCategory::where([
            ...
            $totalFiltered = ExtraCategory::where([
                ['category_name', 'LIKE', "%{$search}%"],
                ['is_active', true]
            ])->count();
        }
        ...
        if (!empty($categories)) {
            foreach ($categories as $key => $category) {
                ...
                $nestedData['number_of_product'] = $category->count();
    ...
    public function extraCategories(Request $request)
    {
        $lims_product_data = Product::where([
        ...
        return view('backend.sale.extra-categories',compact('extraCategories'));
    }
}