...
class SettingController extends Controller
{
    ...
    public function workinghours() {
        $workinghours = Workinghour::where('user_id', Auth::user()->id )->first();
        return view( 'backend.setting.workinghours_setting', compact('workinghours') );
    }

    public function workinghoursSetting( Request $request ) {
        Workinghour::updateOrCreate( [ 'user_id' => Auth::user()->id ], [ 'enable_workinghours' => $request->workinghours_check ==true ? 1 : 0,
        'sunday'=> $request->sunday_week_check == true ? 1 : 0, 'sunday_first_time_start'=> $request->sunday_first_time_start, 'sunday_first_time_end'=> $request->sunday_first_time_end, 'sunday_second_time_enable' => $request->sunday_second_time_enable == "true" || $request->sunday_second_time_enable==1 ? 1 : 0, 'sunday_second_time_start' => $request->sunday_second_time_start, 'sunday_second_time_end' => $request->sunday_second_time_end,
        'monday'=> $request->monday_week_check == true ? 1 : 0, 'monday_first_time_start'=> $request->monday_first_time_start, 'monday_first_time_end'=> $request->monday_first_time_end, 'monday_second_time_enable' => $request->monday_second_time_enable == "true" || $request->monday_second_time_enable==1 ? 1 : 0, 'monday_second_time_start' => $request->monday_second_time_start, 'monday_second_time_end' => $request->monday_second_time_end,
        'tuesday'=> $request->tuesday_week_check == true ? 1 : 0, 'tuesday_first_time_start'=> $request->tuesday_first_time_start, 'tuesday_first_time_end'=> $request->tuesday_first_time_end, 'tuesday_second_time_enable' => $request->tuesday_second_time_enable == "true" || $request->tuesday_second_time_enable==1 ? 1 : 0, 'tuesday_second_time_start' => $request->tuesday_second_time_start, 'tuesday_second_time_end' => $request->tuesday_second_time_end,
        'wednesday'=> $request->wednesday_week_check == true ? 1 : 0, 'wednesday_first_time_start'=> $request->wednesday_first_time_start, 'wednesday_first_time_end'=> $request->wednesday_first_time_end, 'wednesday_second_time_enable' => $request->wednesday_second_time_enable == "true" || $request->wednesday_second_time_enable==1 ? 1 : 0, 'wednesday_second_time_start' => $request->wednesday_second_time_start, 'wednesday_second_time_end' => $request->wednesday_second_time_end,
        'thursday'=> $request->thursday_week_check == true ? 1 : 0, 'thursday_first_time_start'=> $request->thursday_first_time_start, 'thursday_first_time_end'=> $request->thursday_first_time_end, 'thursday_second_time_enable' => $request->thursday_second_time_enable == "true" || $request->thursday_second_time_enable==1 ? 1 : 0, 'thursday_second_time_start' => $request->thursday_second_time_start, 'thursday_second_time_end' => $request->thursday_second_time_end,
        'friday'=> $request->friday_week_check == true ? 1 : 0, 'friday_first_time_start'=> $request->friday_first_time_start, 'friday_first_time_end'=> $request->friday_first_time_end, 'friday_second_time_enable' => $request->friday_second_time_enable == "true" || $request->friday_second_time_enable ==1 ? 1 : 0, 'friday_second_time_start' => $request->friday_second_time_start, 'friday_second_time_end' => $request->friday_second_time_end,
        'saturday'=> $request->saturday_week_check == true ? 1 : 0, 'saturday_first_time_start'=> $request->saturday_first_time_start, 'saturday_first_time_end'=> $request->saturday_first_time_end, 'saturday_second_time_enable' => $request->saturday_second_time_enable == "true" || $request->saturday_second_time_enable==1 ? 1 : 0, 'saturday_second_time_start' => $request->saturday_second_time_start, 'saturday_second_time_end' => $request->saturday_second_time_end,
        ] );
        // var_dump($request->wednesday_second_time_enable );
        return back()->with( 'message', 'Saved successfully' );
    }

    public function action_buttons()
    {
        $actionbuttons = ActionButton::all();
        return view('backend.setting.action_buttons', compact('actionbuttons'));
    }

    public function addActionButton(Request $request)
    {
        $actionButton = new ActionButton();

        $actionButton->name = $request->input('name');
        $actionButton->icon = $request -> input('icon') ? $request -> input('icon') : "";
        $actionButton->color = $request -> input('color') ? $request -> input('color') : "";
        $actionButton->actionlink = $request -> input('actionlink');
        $actionButton->status = true;
        // Add more fields as needed
        $actionButton->save();

        return response()->json(['add_action_buton' => $actionButton]);
    }

    public function changeStatusActionButton(Request $request, $id){
        ...
        ActionButton::where('id', $id)->update([
            'status' => $status
            // Add more fields as needed
        ]);

        return response()->json(['message' => 'action button status changed successfully']);
    }

    public function updateActionButton(Request $request, $id){
        ActionButton::where('id', $id)->update([
    ...
    {
        $action_button = ActionButton::find($id);
        if ($action_button) {
            $action_button->delete();
            return redirect('setting/action_buttons');
        } else {
            return redirect('setting/action_buttons');
        }
    }
    ...
    public function smswhatsapp_setting() {
        $setting = WhatsappSmsSetting::first();
        return view('backend.setting.whatsappsms', compact('setting'));
    }

    public function settingsmswhatsapp_setting_store(Request $request) {
        $setting = WhatsappSmsSetting::first();
        if($setting) {
    ...
    public function posSetting()
    {
        ...
        if($lims_pos_setting_data)
            ...
            $options = [];
        $methods = CustomMethod::get();
        return view('backend.setting.pos_setting', compact('methods', 'lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data','options'));
    }

    public function pos_setting_custom_method_delete($id) {
        CustomMethod::find($id)->delete();
        return redirect()->back()->with('message', 'Deleted Successfully');
    }

    public function posSettingStore(Request $request)
    {
        CustomMethod::where('id', '!=', 0)->update(['active' => 0]);
        foreach($request->coptions as $copt) {
            $cmethod = CustomMethod::where('id', $copt)->update(['active' => 1]);
        }
        /*if(!env('USER_VERIFIED'))
            return redirect()->back()->with('not_permitted', 'This feature is disable for demo!');*/
    ...
    public function custom_method_store(Request $request) {
        CustomMethod::create([
            'name' => $request->title,
        ]);
        return redirect()->back()->with('message', 'Methods updated successfully');
    }

    public function menu_payment_view(){
        $lims_menu_payment_settings = MenuPayment::latest()->first();

        if($lims_menu_payment_settings){
            $options = explode(',', $lims_menu_payment_settings->payment_options);
        }else{
            $options = [];
        }
        ...
        $lims_biller_list=[];
        $lims_warehouse_list =[];
        return view('backend.setting.payment', compact( 'lims_warehouse_list','lims_pos_setting_data','lims_biller_list',  'lims_customer_list','lims_menu_payment_settings', 'options',  'methods'));
    }

    public function menu_payment_create(Request $request){
        CustomMethod::where('id', '!=', 0)->update(['is_online' => 0]);
        foreach($request->coptions as $copt) {
            $cmethod = CustomMethod::where('id', $copt)->update(['is_online' => 1]);
        }

        $data = $request->all();

        //imploding payment options of digital menu
        if(isset($data['options'])){
            $options = implode(',',$data['options']);
        } else {
            $options = '"none"';
        ...
        $menu_payment = MenuPayment::firstOrNew(['id' => 1]);
        $menu_payment->id = 1;
        $menu_payment->name = "Digital Menu";
        $menu_payment->payment_options = $options;
        //save the data gotten
        $menu_payment->save();
        return redirect()->back()->with('message', 'Menu Payment Options Updated successfully');
    }
}