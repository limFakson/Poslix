...
class SaleController extends Controller
{
    ...
    public function sendsms($id) {
        $cust = Sale::find($id)->customer_id;
        $lims_customer_data = Customer::find($cust);
        $wssetting = WhatsappSmsSetting::first();
        if($wssetting) {
            if($wssetting->whatsapp) {
                $account_sid = env('ACCOUNT_SID');
                $auth_token = env('AUTH_TOKEN');
                $twilio_phone_number = env('Twilio_Number');
                $wsphone = $lims_customer_data->phone_number;
                $messagew = $wssetting->template." ".url("view/sales/".$id);
                try{
    ...
    public function sendwp($id) {
        $cust = Sale::find($id)->customer_id;
        $lims_customer_data = Customer::find($cust);
        $wssetting = WhatsappSmsSetting::first();
        if($wssetting) {
            if($wssetting->whatsapp == 1) {
                $wsphone = $lims_customer_data->phone_number;
                $messagew = $wssetting->template." ".url("view/sales/".$id);
                $urlw = "https://wa.me/".$wsphone."?text=".$messagew;
                return redirect(url($urlw));
            }
        }
    }
    ...
    public function saleData(Request $request)
    {
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['time'] = Carbon::parse($sale->created_at)->format('H:i A');
                ...
                if($sale->sale_type){
                    $nestedData['sale_type'] = $sale->sale_type. '<br>'.$sale->order_type;
                }
                ...
                if($wssetting) {
                    if($wssetting->whatsapp == 1)
                    {
                        $wslinks .= '<li><a href="'.route('sale.sendwp', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> '."Share Whatsapp".'</a></li>';
                    }

                    if($wssetting->sms == 1)
                    {
                        $wslinks .= '<li><a href="'.route('sale.sendsms', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> '."Send SMS".'</a></li>';
                    }
                }
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                ...
                                <li><a href="'.route('sale.viewinvoice', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('file.View Invoice').'</a></li>
    ...
    public function store(Request $request)
    {
        ...
        if($wssetting) {
            ...
            //     $messagew = $wssetting->template." ".url("view/sales/".$lims_sale_data->id);
            //     $urlw = "https://wa.me/".$wsphone."?text=".$messagew;
            //     return redirect('pos')->with('message', $message)->with('urlw', $urlw)->with('lc', true);
            // }
        }
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')) {
            ...
            $numberOfInvoice = Sale::count();
            $custom_fields = CustomField::where('belongs_to', 'sale')->get();
            if(isset(request()->newpos) && request()->newpos==0)
            {
            ...
            return view('backend.sale.pos-2024', compact('currency_list','role','all_permission', 'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 'lims_reward_point_setting_data', 'lims_product_list', 'product_number', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'options', 'lims_brand_list', 'lims_category_list', 'lims_table_list', 'recent_sale', 'recent_draft', 'lims_coupon_list', 'flag', 'numberOfInvoice', 'custom_fields'));
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $product[] = $qty;
        $product[] = $lims_product_data->extraCategories->count() > 0;
        if($lims_product_data->extraCategories->count() > 0)
        {
            $product[] = $lims_product_data->extraCategories->pluck('name','id');
        }
        else
            $product[] = [];
    ...
    public function genInvoiceView($id)
    {
        $lims_sale_data = Sale::find($id);
        $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
        if(cache()->has('biller_list'))
        ...
        if(is_null($lims_sale_data->exchange_rate))
        {
            ...
            if(cache()->has('currency')) {
                ...
                $currency_code = \App\Models\Currency::find($settingData->currency)->code;
        ...
        if($lims_pos_setting_data->invoice_option == 'A4') {
            return view('backend.sale.a4_invoice', compact('lims_sale_data', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'paid_by_info', 'sale_custom_fields', 'customer_custom_fields', 'product_custom_fields', 'qrText', 'general_setting'));
        }
        else{
            return view('backend.sale.invoice', compact('lims_sale_data', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'sale_custom_fields', 'customer_custom_fields', 'product_custom_fields', 'qrText', 'general_setting'));
        }
    }
