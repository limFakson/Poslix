...
class PosLivewire extends Component
{
    ...
    public $lims_pos_setting_data, $lims_table_list, $lims_warehouse_list, $lims_biller_list, $lims_customer_list, $custom_fields, $lims_product_array, $general_setting, $options;
    ...
    public $hasExtra = false, $added = false;
    ...
    public function render()
    {
        ...
        foreach ($this->items as $index => $item) {
            ...
            $this->items[$index]['total_tax'] = $item['total_tax'];
                ...
                if ($extras) {
                    foreach ($extras as $extra) {
                        ...
                        if (!empty($this->items[$index]['extra_names']))
                            $this->items[$index]['extra_names'] .= ",";
                        $this->items[$index]['extra_names'] = $this->items[$index]['extra_names'] . ' ' . $extra->name;
                        $totalExtra += $item['qty'] * ($extra->price + $tax);
                    }
                }
            }
        ...
        $this->grand_total = (collect($this->items)->sum('total') + $this->shipping_cost) - $this->total_discount;

        $this->order_tax = 0;
        if ($this->order_tax_rate !== '0') {
            $tax_rate_order = explode('|', $this->order_tax_rate);
            $this->order_tax = $this->grand_total * ($tax_rate_order[0] / 100);
        }
    ...
    public function updatedPaymentData()
    {
        ...
        if ($this->payment_data['paying_amount'] == 0 && $this->change_amount == 0)
    ...
    public function addToCart($product)
    {
        ...
        //Log::debug($product);
        $items = $this->items;
        $index = $this->searchItemByCode($this->items, $product->code);
        ...
        //$this->dispatch('open-qty-modal', $this->qtyIndex);
...
*/
        ...
        $data = [
            ...
            'total_tax' => $this->order_tax,
            'total_price' => collect($this->items)->sum('total'),
            'item' => count($this->items),
            'order_tax' => collect($this->items)->sum('total_tax'),
            'grand_total' => $this->grand_total,
            'used_points' => NULL,
            'coupon_discount' => collect($this->items)->sum('total'),
            'sale_status' => '1',
        ...
        foreach ($this->items as $i => $item) {
            ...
            if (count($sale_unit)>0 && $sale_unit[$i] != 'n/a') {
